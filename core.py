...
if getattr(sys, 'frozen', False):
    _CONFIG_PATH = os.path.join(sys._MEIPASS, "config.json")
else:
...
def _get_majors_and_noise():
    ...
    majors_config = config.get("major_words", {})
    # 字典的键是专业标准名称，值是关键词列表
    standard_names = list(majors_config.keys())
    keyword_lists = list(majors_config.values())
    return standard_names, keyword_lists, noise_words, abbreviations
...
def get_major_class(filename_str):
    ...
    """
    # 从配置文件加载专业关键词、简称和干扰词
    standard_names, keyword_lists, words_to_remove, abbreviations = _get_majors_and_noise()

    # 初始化返回值
    major_class = ""  # 最终返回的"专业简称+班级"字符串
    ...
    for i, keywords in enumerate(keyword_lists):
        for keyword in keywords:
            ...
            if idx != -1:
                # 找到匹配的专业关键词，使用该专业的简称
                major_class = abbreviations.get(standard_names[i], standard_names[i])
                found_major = True
